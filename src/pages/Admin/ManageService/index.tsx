...
function ManageService() {
  ...
  useEffect(() => {
    ...
    (async () => {
      let data = await StaticPageDataService.getStaticPageDataById(2);
      console.log(data);
      setData(data?.data ?? '');
    })();
  }, [editorLoaded]);
  ...
  const onClickSave = async () => {
    setIsSaving(true);
    try {
      await StaticPageDataService.updateStaticPageData(2, data);
      toast.success('Service content saved successfully!');
    } catch (e) {
      toast.error('Error while saving Service content');
    }
    setIsSaving(false);
  };
      ...
      <Container sx={{}}>
        <Grid2 sx={{ flex: 1, pt: 8, justifyContent: 'center' }} container spacing={2}>
          <Grid2 sx={{ ml: 0, flex: 1 }}>
            <Typography sx={{ mt: 2, fontSize: 24, fontWeight: '500' }}>{'Service'}</Typography>
            <CKEditor
