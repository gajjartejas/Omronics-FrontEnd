...
function UpdateProduct() {
    ...
    (async () => {
      ...
      try {
        ...
        if (productInfo?.manufacturerId) {
          selectedManufacturerId.current = { id: productInfo?.manufacturerId };
        }
        await getManufacturers(productInfo?.manufacturerId);

        //Categories
        const categoriesIds = productInfo?.categories.map(v => v.id);
        getCategories(categoriesIds);
        selectedCategoryIds.current = productInfo?.categories.map(v => {
          return { id: v.id };
        });
        ...
        let pImages = productInfo.images.map(v => {
          return {
            id: v.id,
            dataURL: Config.Constants.IMAGE_PATH + v.url!,
  ...
  }, [editorLoaded, productId]);

  const getCategories = async (selectedIds: number[]) => {
    ...
    categories?.forEach(
      aData =>
        (hashTable[aData.id] = {
          ...
          value: aData.name,
          children: [],
          checked: selectedIds.includes(aData.id),
        }),
    );
  ...
  const getManufacturers = async (selectedId: number | null) => {
  ...
  const onClickSave = async () => {
    ...
    const newProduct: IBaseProduct = {
      ...
      categories: { set: selectedCategoryIds.current },
    ...
    try {
      const numId = Number(productId);
      const result = await ProductService.updateProduct(numId, newProduct);
      if (result) {
  ...
  const onClickUploadFile = () => {
    let isAllFileUploaded =
      files.filter(v => v.status !== UploadStatus.FINISH && v.status !== UploadStatus.REMOTE).length === 0;
    ...
    <div>
      <Container sx={{}}>
            ...
            <Grid container direction="row" justifyContent="left" alignItems="center" sx={{}}>
              ...
              {files.map((file, index) => (
                <Components.ResourcesCard
                  ...
                  resourcesType={files[index].type}
                  mode={'show_file'}
        ...
        <Button onClick={onClickSave} sx={{ mt: 2, mb: 2 }} variant="contained">
          {isProductCreating ? 'Updating Product, Please Wait...' : 'UPDATE PRODUCT'}
        </Button>
      </Container>
    </div>
  );
